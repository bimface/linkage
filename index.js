...
export default class Model2d3d extends Component {
    constructor() {
        super();
        this.state = {
            cHeight: document.documentElement.clientHeight - 90 - 20,
            viewToken:'',
            showErrorNote:false,
            isConnecting: false,
            isPopConnect:false,
    ...
    matchPos = () => {
        ...
        const endX = viewerDwg.getViewer().startX - (centerClientPos.x - viewerDwg.getViewer().width / 2);
        const endY = viewerDwg.getViewer().startY - (centerClientPos.y - viewerDwg.getViewer().height / 2);
        viewerDwg.getViewer().panTo(endX,endY)
    }
    ...
    addLabel = (position, type) => {
        ...
        // 添加自定义标签
        if(type == '3D') {
            casheLabelId.push(customItem.id);
            casheLabelId.length == 3 ? drawable3dContainer.removeItemById(casheLabelId.shift()) : '';
    ...
    locateTo3D = (el) => {
        ...
        const delta2D = Math.pow(secPos2D.x - firstPos2D.x, 2) + Math.pow(secPos2D.y - firstPos2D.y, 2);
        const delta3D = Math.pow(secPos3D.x - firstPos3D.x, 2) + Math.pow(secPos3D.y - firstPos3D.y, 2);
        const delta = Math.sqrt(delta2D / delta3D);

        const xPos = firstPos3D.x + delta * (el.worldPosition.x - firstPos2D.x);
        const yPos = firstPos3D.y + delta * (el.worldPosition.y - firstPos2D.y);
    ...
    setCameraPos = (cameraStatus) => {
        ...
        cameraWrap.style.top = posY -15;
        cameraWrap.style.left = posX - 7;
        ...
        return (
            <div className="drawing-list">
                <div className="title">
                  ...
                  <div className="brief">Revit正向出图产生的图纸，即由模型生成的图纸，可以与三维模型进行构件联动及位置联动。</div>
                </div>
                <ul className="detail">
        ...
        return(
            <div className="drawing-list">
                <div className="title">
                    关联的图纸
                    <i className="iconfont icon-information-"></i>
                    <div className="brief">CAD绘制的图纸，翻模所用的材料。通过本demo的关联图纸功能，可实现图纸与三维模型的位置联动。</div>
                    { floors ? 
